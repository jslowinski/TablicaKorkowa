...
abstract class MySwipeHelper(context: Context,
    ...
    private val onTouchListener = View.OnTouchListener{ _, motionEvent ->
        if(swipePosistion < 0) return@OnTouchListener false
        val point = Point(motionEvent.rawX.toInt(), motionEvent.rawY.toInt())
        val swipeViewHolder = recyclerView.findViewHolderForAdapterPosition(swipePosistion)
                ...
                motionEvent.action == MotionEvent.ACTION_UP)
        {
            if (rect.top < point.y && rect.bottom > point.y)
                ...
                removeQueue.add(swipePosistion)
                swipePosistion = -1
                recoverySwipeItem()
            }
        }
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
        ...
        if (swipePosistion != pos)
            removeQueue.add(swipePosistion)
        swipePosistion = pos
        if (buttonBuffer.containsKey(swipePosistion))
            buttonList = buttonBuffer[swipePosistion]
        else
            buttonList!!.clear()
        buttonBuffer.clear()
